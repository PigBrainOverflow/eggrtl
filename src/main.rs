...
use core_relations::{make_external_func, ColumnId, ContainerValue, Database, DisplacedTable, Rebuilder, SortedWritesTable, TableId, Value};
...
fn dump_tables(db: &Database, tables: &[TableId]) {
    db.container_values().for_each::<VecContainer>(|vec, expr| {
        println!("Container {:?}: {:?}", expr, vec);
    });

    // let table = db.get_table(wires);
    // let rows = table.all();
